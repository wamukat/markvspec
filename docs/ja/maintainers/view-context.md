...
短縮形として Action 直下に効果 block を残す案は採用しません。書きやすくはなりますが、レビュー時に「処理」と「結果」の対応が見えにくくなり、parser も複数の等価表現を正規化する必要が出ます。MarkVSpec では、読み書きの省略よりも、レビューしやすい単一の canonical form を優先します。
