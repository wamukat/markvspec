...
This document defines the verification gate for the MarkVSpec `0.6.3` release.
...
- [ ] Package metadata and the VSIX packaging script produce a
  `dist/markvspec-0.6.3.vsix` artifact for this release.
...
package dependencies. For the `0.6.3` release, publish packages in this order:
...
Use exact ranges such as `"@markvspec/core": "0.6.3"` in published package
