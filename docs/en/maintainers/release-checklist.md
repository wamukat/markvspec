...
This document defines the verification gate for the MarkVSpec `0.7.1` release.
...
- [ ] Package metadata and the VSIX packaging script produce a
  `dist/markvspec-0.7.1.vsix` artifact for this release.
...
package dependencies. For the `0.7.1` release, publish packages in this order:
...
Use exact ranges such as `"@markvspec/core": "0.7.1"` in published package
