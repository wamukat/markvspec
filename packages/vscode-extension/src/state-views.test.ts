...
`;
  ...
  assert.match(html, new RegExp(`<td>${detailElementRef("E-SubmitButton", "E-SubmitButton")}</td><td>label</td><td>Submit &amp; Continue</td><td>-</td><td>${plainCodePattern("fixed")}</td><td>${defaultAlwaysPattern()}</td><td><ul class="spec-list"><li>enabled: not ${markerBadge("E-メールアドレス入力", "element")} is empty</li></ul></td>`));
