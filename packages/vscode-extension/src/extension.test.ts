...
test("renders generated design document sections without launching VS Code", () => {
  ...
  assert.match(html, new RegExp(`email: ${sourceCodePattern("${model.email}")}`));
  assert.match(html, new RegExp(`rememberMe: ${sourceCodePattern("${model.rememberMe}")}`));
  assert.match(html, new RegExp(`<td>${markerBadge("L2", "layout")}</td><td>${detailIdRef("L-LoginForm")}</td><td>stack</td><td><ul class="spec-list"><li>disabled: wait-auth</li></ul></td>`));
  ...
  assert.match(html, new RegExp(`set state ${docLabel("validation-error", "state")}[\\s\\S]*content Required field message`));
  assert.match(html, new RegExp(`set state ${docLabel("auth-error", "state")}[\\s\\S]*content Authentication error message`));
  ...
  assert.match(actionDetailsSection, new RegExp(`email: ${sourceCodePattern("${model.email}")}`));
  assert.match(actionDetailsSection, new RegExp(`rememberMe: ${sourceCodePattern("${model.rememberMe}")}`));
  ...
  assert.match(submitActionDetail, new RegExp(`<strong>${docLabel("sent", "result")}</strong>[\\s\\S]*effect set state ${docLabel("wait-auth", "state")}`));
  ...
  assert.match(waitAuthSection, new RegExp(`<td>${markerBadge("11", "element")}</td><td>${detailIdRef("E-AuthSpinner")}</td><td>label</td><td>Signing in\\.\\.\\.</td><td></td><td></td><td><ul class="spec-list"><li>visible: wait-auth</li></ul></td><td></td>`));
  ...
  const desktopWaitAuthSection = viewportStateSection(html, "wait-auth", "desktop");
