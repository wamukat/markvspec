...
4. CI、スクリプト、複数ファイルの一括出力、project document list では CLI を使う。
...
ソースファイルを parse/validate し、構文上の不足や参照の問題を確認します。warning でも CI を
...
プロジェクトの意図、メモ、document list、遷移図を一緒に読みたい場合はプロジェクトプレビューを使います。
...
- 入力を省略すると、current directory から `.vspec.md` / `.vspec.project.md` を探します。スクリプトでは明示的なファイルや glob を指定してください。
...
Front Matter の `messages` は MarkVSpec ファイルからの相対 path で、同じ directory の内側に
