...
Project preview renders the project lead and `## Notes` so reviewers can read the project intent beside the screen/template list and transition graph. HTML/PDF export can also accept a project file; it loads the listed screens and packages those screen specifications into one artifact. CLI [document-list export](/markvspec/en/reference/cli/) intentionally keeps the output compact and does not include long project lead / notes prose.

Related:

- [Preview](/markvspec/en/start/preview/): project preview contents.
- [Export](/markvspec/en/start/export/): project HTML/PDF export.
- [CLI](/markvspec/en/reference/cli/): `export document-list` for a compact inventory.
