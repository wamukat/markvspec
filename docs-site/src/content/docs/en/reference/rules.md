...
Use Business Rules when the decision depends on product meaning: permissions, account status, plan restrictions, stock, or a business decision that reads multiple values. Do not use this section for basic input shape such as required, format, min, max, or simple field comparison. Use [Validations](/markvspec/en/reference/validations/) for those checks.
...
- Put simple multi-field comparisons in [Validations](/markvspec/en/reference/validations/) as `## Cross-field Validations`.
